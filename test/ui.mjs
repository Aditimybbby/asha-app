...
try{
 ...
 await step('Visit editing requires actual visit date and preserves outcome',async()=>{
  await action('nav-visits').click();await action('edit-visit').click();await page.locator('#modal select[name="status"]').selectOption('Completed');await page.locator('#record-form button[type="submit"]').click();assert.ok(await page.locator('#form-error').textContent().then(s=>s.includes('actual date')));await modalFill('actualDate',new Date().toLocaleDateString('en-CA',{timeZone:'Asia/Kolkata'}));await saveForm();
 });
 ...
 await step('Mobile layout fits a 390px viewport and navigation works',async()=>{
  await page.setViewportSize({width:390,height:844});await page.locator('.mobile-nav [data-action="nav-home"]').click();assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));await page.screenshot({path:'artifacts/02-mobile-overview.png',fullPage:true});await page.locator('.mobile-nav [data-action="nav-households"]').click();assert.ok(await page.locator('h1').textContent().then(s=>s==='Families'));await page.screenshot({path:'artifacts/03-mobile-families.png',fullPage:true});
 });
 await page.close();
